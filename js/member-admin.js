...
(function(){
  ...
  function renderUserItem(u){
    const li = el('li', 'p-3 rounded bg-gray-50 dark:bg-gray-800 flex items-center justify-between');
    const left = el('div'); left.innerHTML = `<div class="font-semibold">${u.username}</div><div class="text-xs text-gray-500">${u.email||''} · 建立：${fmtDate(u.createdAt)}${(typeof u.completion==='number')?` · 完成度：${Math.round(u.completion)}%`:''}</div>`;
    const btn = el('button', 'btn-soft btn-green'); btn.textContent = '查看';
  ...
  async function loadProfile(username){
    ...
    function row(k,v){ return `<div class="flex items-center justify-between py-1"><div class="text-gray-500">${k}</div><div class="font-medium">${v||''}</div></div>`; }
    function list(title, items){
      if (!items.length) return '';
      return `<section class="mt-4"><h3 class="text-lg font-semibold mb-2">${title}</h3><ul class="space-y-2">${items.map(it=>`<li class="p-3 rounded bg-gray-50 dark:bg-gray-800 flex items-center justify-between"><span>${it.title||it.name||''}${it.date?` <span class='text-xs text-gray-500'>(${it.date})</span>`:''}</span></li>`).join('')}</ul></section>`;
    }
