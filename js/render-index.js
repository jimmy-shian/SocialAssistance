(function () {
  ...
  if (data.hero) {
    ...
    if (c) {
      ...
      c.innerHTML = `
        <span class="block text-[var(--primary)] font-bold mb-2 tracking-widest">${h.label || 'SOUND CORE STUDIO'}</span>
...
if (data.sdgs && Array.isArray(data.sdgs)) {
  ...
  if (!document.getElementById('sdg-anim-style')) {
    ...
    style.id = 'sdg-anim-style';
    style.textContent = `
      .sdg-icon-wrapper svg path {
        fill: transparent;
        stroke: currentColor;
        stroke-width: 2;
  ...
  const items = sdgsGrid.querySelectorAll('.sdgs-item');

  // 1️⃣ First view: IntersectionObserver（只播一次）
  if ('IntersectionObserver' in window) {
    ...
    items.forEach(el => playOnce(el));
  }

  // 2️⃣ Hover behavior（不抽搐版）
  items.forEach(el => {
