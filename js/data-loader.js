// Data Loader for Wix integration
(function () {
  const AppConfig = window.AppConfig || { API_BASE: '', endpoints: {}, datasets: {}, versionCacheKey: 'app_data_version' };
  ...
  function getUrl(endpointKey) {
    const epName = EP[endpointKey] || endpointKey;
    return BASE.replace(/\/$/, '') + '/' + epName.replace(/^\//, '');
  }
  ...
  async function postJson(url, payload) {
    ...
    const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body, credentials: 'omit' });
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    return await resp.json();
  }
  ...
  async function fetchData(key) {
    ...
    const url = getUrl('data') + '?key=' + encodeURIComponent(key) + '&v=' + encodeURIComponent(v);
  ...
  async function secureRead(key, authToken) {
    ...
    const res = await postJson(url, { key, token: t, nonce: mkNonce() });
  ...
  async function login(username, password) {
    if (!BASE || !EP.login) throw new Error('未設定 API_BASE');
