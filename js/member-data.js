...
(function () {
  const AppConfig = window.AppConfig || { API_BASE: '', endpoints: { profileRead: 'profileRead', profileUpdate: 'profileUpdate' } };
  ...
  function hasMeaningfulContent(p) {
    ...
    try {
      ...
      const selfFilled = ['interests', 'strengths', 'goals', 'teacherComments'].some(k => (s[k] || '').toString().trim());
  ...
  async function loadProfile(username) {
    ...
    if (base) {
      try {
            ...
            try {
              ...
              if (!hasMeaningfulContent(remote) && localRaw) {
                ...
                if (hasMeaningfulContent(localObj)) {
                  await saveProfile(username, localObj); // 嘗試同步到 Wix
                  return localObj;
                }
              }
            } catch (e) { }
        ...
        console.warn('Backend profile fetch failed, falling back to local', e);
      }
    }
    // Fallback local
  ...
  async function saveProfile(username, profile) {
    ...
    if (base) {
      try {
        ...
        const resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token, username, profile }) });
        if (resp.ok) {
          const data = await resp.json();
          if (data && data.ok) return { ok: true };
        }
      } catch (e) {
        console.warn('Backend profile save failed, saving local instead', e);
      }
    }
