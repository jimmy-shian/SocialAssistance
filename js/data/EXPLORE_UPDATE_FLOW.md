...
為優化載入效能，網頁統一使用 WebP 格式圖片。請使用專案內建的 Python 工具進行批次轉換並自動刪除原始 JPG/PNG 檔案：

```powershell
# 請在專案根目錄下使用 PowerShell 執行：
& "C:\Users\user\venv\Scripts\python.exe" .\tools\convert_images_to_webp.py --dir "img\explore\【探索對象名稱】" --delete
```

執行後，所有資料夾底下的圖片會自動轉換為 `.webp` 並清除原檔。
