...
(function () {
  ...
  function timelineBlogHref(item) {

    const directHref = item && (item.relatedBlogHref || item.blogHref || item.blogLink);

    if (directHref) return String(directHref);

    const postId = item && (item.relatedBlogPostId || item.blogPostId || item.blogId);

    return postId ? `./blog.html?id=${encodeURIComponent(postId)}` : '';

  }

  function timelineBlogLabel(item) {

    return /Q[＆&]A|問答/.test(String((item && item.title) || '')) ? '閱讀人物造訪' : '閱讀相關文章';
...
function open() {
  ...
  function render(provider) {
      ...
      <section aria-labelledby="sec-timeline" class="category-section mb-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          ${(provider.timeline || []).map((item, idx) => {
            ...
            const hasImgs = Array.isArray(item.images) && item.images.length;

            const stepImg = hasImgs ? item.images[0] : ((provider.images && provider.images.length) ? provider.images[idx % provider.images.length] : fallbackImages[idx % fallbackImages.length]);

            const relatedBlogHref = timelineBlogHref(item);

            const relatedBlogLabel = timelineBlogLabel(item);

            

            const mediaBlock = `

              <div class="rounded-lg overflow-hidden timeline-media cursor-pointer" data-images="${(item.images || []).join('|')}">

                <figure class="image-frame image-frame--card image-frame--rounded aspect-[4/3] overflow-hidden">

                  <img decoding="async" src="${stepImg}" alt="${esc(item.title || '課程步驟')}" class="w-full h-full object-cover transition-transform duration-500 hover:scale-105" loading="lazy" />
            ...
            return `
              ...
              <article class="p-4 rounded-xl card-dynamic-bg hover:shadow-lg transition-all duration-300 group flex flex-col h-full border border-[var(--border)]">
                ...
                <div class="flex-1 flex flex-col mt-2">
                  ...
                  <h3 class="font-bold text-xl mb-2 text-[var(--text-primary)] group-hover:text-[var(--primary)] transition-colors">${esc(item.title || '探索步驟')}</h3>

                  <p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed flex-1">${esc(item.detail || '')}</p>

                  ${relatedBlogHref ? `

                    <a class="inline-flex items-center gap-1 mt-4 text-sm font-bold text-[var(--primary)] hover:text-[var(--primary-light)]" href="${esc(relatedBlogHref)}">

                      ${esc(relatedBlogLabel)} <span aria-hidden="true">→</span>
    ...
    // Timeline: bind lightbox on media

    try {

      const timelineRoot = document.getElementById('sec-timeline')?.parentElement;

      if (timelineRoot) {

        timelineRoot.querySelectorAll('.timeline-media').forEach(box => {
      ...
      console.warn('Timeline lightbox bind failed:', e);
